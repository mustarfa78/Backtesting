...
def main() -> None:
    ...
    while True:
        ...
        if announcement.source_exchange == "Bitget":
            if not allowed:
        ...
        for name in adapter_stats.get("counts", {}):
            if per_source_rows.get(name, 0) > 0:
                continue
            if adapter_stats.get("errors", {}).get(name):
                reason = adapter_stats["errors"][name]
            elif adapter_stats["counts"].get(name, 0) == 0:
